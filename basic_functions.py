...
def graph_check(graph):
    ...
    if type(graph[0]) != type(s):
        raise Exception("Invalid type of adjacency matrix!")
    if type(graph[1]) != type(s):
        raise Exception("Invalid type of weights matrix!")
    if type(graph[2]) != list:
        raise Exception("Invalid type of vertex list!")
    print("Correct graph representation")

    # check if graph is 2D matrix
    if len(graph[1].shape) != 2:
        raise Exception("Invalid weights matrix dimension!")
    if len(graph[0].shape) != 2:
        raise Exception("Invalid adjacency matrix dimension!")
    if graph[0].shape[0] != graph[0].shape[1]:
        raise Exception("Invalid adjacency matrix shape!")
    if graph[1].shape[0] != graph[1].shape[1]:
        raise Exception("Invalid weights matrix shape!")
    ...
    if graph[1].shape != graph[0].shape:
        raise Exception(
            "Weights matrix and connection matrix have different dimensions!"
        )

    # Check node amount
    if len(graph[2]) != len(graph[0]):
        raise Exception("Invalid graph's nodes amount")
    ...
    for step in path[0]:
        for point_name in points_names:
            if point_name == step:
                cords.append(
                    [
                        points[points_names.index(point_name)].x + offset_x,
                        points[points_names.index(point_name)].y + offset_y,
                    ]
                )
...
def draw_info(point, offset_x, offset_y):
    ...
    text_rendering(
        ...
        OFF_WHITE,
        (
            point.x + offset_x - 2 * point.radius + (8 * len(point.name)) / 2,
            point.y + offset_y + 5 * point.radius + (5 * point.radius) / 2,
        ),
        BASIC_FONT,
    )
...
def check_point_collision(point, offset_x, offset_y):
    rect = pygame.Rect(
        point.x + offset_x, point.y + offset_y, 3 * point.radius, 3 * point.radius
    )
...
def draw_help_info(button, x, y):
    ...
    pygame.draw.rect(WIN, OFF_WHITE, pygame.Rect(x + 600, y, 100, 100))


def animate_arrow(counter):
    # animate arrowR
    pygame.draw.polygon(WIN, LIGHT_GREEN, [(295, 327), (310, 339), (310, 315)])
    pygame.draw.rect(WIN, LIGHT_GREEN, pygame.Rect(310, 321, 20, 12))

    # animate arrowL
    pygame.draw.polygon(WIN, LIGHT_GREEN, [(160, 327), (145, 339), (145, 315)])
    pygame.draw.rect(WIN, LIGHT_GREEN, pygame.Rect(125, 321, 20, 12))

    counter += 1
    if counter > 150:
        # animate arrowR
        pygame.draw.rect(WIN, MENU_GREEN, pygame.Rect(295, 315, 35, 25))

        # animate arrowL
        pygame.draw.rect(WIN, MENU_GREEN, pygame.Rect(125, 315, 40, 25))
...
def draw_suggestions(suggestions):
    pygame.draw.rect(WIN, OFF_WHITE, pygame.Rect(110, 345, 145, len(suggestions) * 15))
...
def convert(string):
    ...
    for s in range(len(string) - 1):
        if string[s] == " ":
            string = string[: s + 1] + string[s + 1].upper() + string[s + 2 :]
...
def draw_details(path):
    ...
    height = 0
    if len(path[0]) <= 5:
        height = len(path[0]) * 26
    else:
        height = len(path[0]) * 17
    pygame.draw.rect(WIN, OFF_WHITE, pygame.Rect(10, 315, 180, height))

    text_rendering(
        "Distance " + "      m", MENU_GREEN, OFF_WHITE, (100, 330), CRAZY_FONT
    )
